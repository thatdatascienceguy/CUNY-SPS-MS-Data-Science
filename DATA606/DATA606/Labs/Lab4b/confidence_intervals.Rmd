...
```{r hist-gr-living-area, echo=TRUE}
hist(samp)
```

Answer:
The typical size is the sample size which is 60 in this case. The distribution of the sampe is bi-modal.
...
Answer: the sample size must be > or equal to 30, the sample observations must be independent and also

the data of the population is nearly normal or not too strongly skewed.
...
No my classmate's confidence interval may not capture the true population due to several things:
  ...
  - the sample mean has a different value
  - the sample standard deviation is different
  - the percent used could be different.
