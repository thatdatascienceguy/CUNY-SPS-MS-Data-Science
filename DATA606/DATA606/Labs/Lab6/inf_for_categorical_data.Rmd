...
Answer: each row corresponds to possible each person sampled in the poll.
...
```{r proportion-atheists, echo=TRUE}
us12_atheists <- subset(us12, response == "atheist")
nrow(us12_atheists)
```
It is close to 5% about 4.9% percent, there might be some standard error or the data are slightly different.
...
Answer: The observations are independent as someone being a atheist does not effect or tell

if someone else is an atheist or not.

Also looking at the number of success and n from the inference function,
np_hat >= 10
...
Answer: the margin of error from using the inference function is 1.96*0.0069 = 0.013524
...
Answer: Conditions are met (independence and np > 10)
```{r atheists-countries, echo=TRUE}
# Canada and Japan
...
inference(japan$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
ME of Canada: 0.01764
ME of Japan: 0.025872
...
Answer: the plot looks like a semi-circle function and when the population porportion is around .5,

the margin of error reaches its maximum.
...
Center and spread (mean and SD)
```{r mean-sd-phats, echo=TRUE}
mean(p_hats)
sd(p_hats)
```
...
All 3 plots follow a normal distribution quite well.

As n gets large, the distribution tends to be more normal and a smaller standard
deviation.

Changing p alters the center (mean) as the mean is p and decreasing or slowly increasing p

makes the distribution skew.
...
Answer: Yes as even though they are point estimates, there is still a margin of error
...
Let $H_{0}$ = no change in atheism index in Spain in the two years that is $p_{spain2005} - p_{spain2012}$ = 0
Let $H_{A}$ = there has been a change in atheism index in Spain that is $p_{spain2005} - p_{spain2012}$ != 0
...
Using the inference function for the two years for Spain the difference of p_hats is slightly above zero so

we are 95% confident that the change of atheism index has slightly change.
...
Let $H_{0}$ = no change in atheism index in the US in the two years that is $p_{US2005} - p_{US2012}$ = 0
Let $H_{A}$ = there has been a change in atheism index in the US that is $p_{US2005} - p_{US2012}$ != 0
...
Using the inference function for the two years for the US the difference of p_hats is slightly above zero so

we are 95% confident that the change of atheism index has slightly change. We fail to reject the null hypothesis and favor
...
Answer: Using the formula for the Margin of Error: ME =  z*SE =
z*sqrt(p(1-p) / n)

Solving for n gives n = p(1-p) / (ME/z*)^2 which is how much people you would need to sample

to get the desired Margin of Error and 95% confidence interval.
