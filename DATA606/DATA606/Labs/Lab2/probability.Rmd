...
Answer: A streak of length 1 means that Kobe had 1 shot followed by a miss. A streak

of 0 means that after the previous miss, got a miss from a shot attempt.
...
Answer: Looking at the distribution it looks like a right-skewed distribution. His typical streak

length was 0 or 1.

Seeing the kobe dataset and seeing 4 baskets (shots) in a row and subtracting the time from the start of the longest

streak to the end of it shows the beginning was around 1:52 in the 2nd quarter, 11:18 the 3rd quarter, 9:52 and 9:23

adding up the times gives 118 seconds (2nd quarter) and 277 seconds = 395 seconds or about 6 min and 50 seconds
...
```{r num-heads, echo=TRUE}
sum(sim_unfair_coin == "heads")
```

Answer: it shows 20 times the unfair coin was set to heads.
...
```{r, streak-lengths, echo=TRUE}
hist(calc_streak(sim_basket))
table(calc_streak(sim_basket))
```

The distribution is heavily right-skewed. The typical length for this simulated

independent shooter is 0 and looking at the table the longest steak of baskets is 6 shots.
...
It would be similar as the probabilities are the same but will have slightly different 

numbers but follow a similar distribution.
...
Kobe's streak lengths follow a similar simulation (right-skewed). This shows that 

Kobe's shooting patterns were independent and as expected it is less likley to get

longer streak lengths.
