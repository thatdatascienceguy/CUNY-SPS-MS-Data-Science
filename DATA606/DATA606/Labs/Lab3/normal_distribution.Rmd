...
Answer
```{r hist-heights, echo=TRUE}
hist(mdims$hgt)
hist(fdims$hgt)
```

The heights of both of them are nearly normal and that the mean and median of men's

heights are slighly higher than women's heights.
...
Yes the density plot does follow a nearly normal distribution.
...
Answer:
```{r, sim-norm-plot, echo=TRUE,eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)
```

Not all points fall on the line for sim_norm when using qqnorm/qqline. This plot

however has much more points on the line or close to it than the female heights.
...
Answer: Yes as by examing the simulated data, each simulated plot has many points at

the line or close to it and few deviations just like the plot of the female heights.

This shows evidence that the data of female heights are nearly normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
Answer: using qqnorm/qqline it is slightly right skewed as doing those plots, the higher

sample quantiles are away from the Q-Q line.

```{r female-knee, echo=TRUE}
qqnorm(fdims$wgt)
qqline(fdims$wgt)
```
...
Answer: Question 1) What is the probability that a female has a height between

140cm to 170cm?

```{r female-height-prob1-emprical, echo=TRUE}
question1_emprical <- sum(fdims$hgt >= 140 & fdims$hgt <= 170) /  length(fdims$hgt)
# empricial probability P(fdims$hgt >= 140 and fdims$hgt <= 170)
question1_emprical
```

```{r female-height-prob1-theoretical, echo=TRUE}
question1_theoretical <- 1-(pnorm(q = 140, mean = fhgtmean, sd = fhgtsd) + 
  pnorm(q = 170, mean = fhgtmean, sd = fhgtsd, lower.tail = FALSE))
question1_theoretical
```

Question 2) What is the probability that a female has a weight greater than 65kg?

```{r femaile-weight-prob2-emprical,echo=TRUE}
question2_emprical <- sum(fdims$wgt > 65) / length(fdims$wgt)
question2_emprical
```

```{r femaile-weight-prob2-theoretical,echo=TRUE}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

question2_theoretical <- pnorm(q = 65, mean = fwgtmean, sd = fwgtsd, lower.tail = FALSE)
question2_theoretical
```

Answering these two questions it shows that the heights are more nearly normally distributed than 

the weights as the emprical and theoretical results were nearly identical.
* * *
...
Answer: This could be because many values in the variables have the same sample quantile

in different values or that the values of the data are nearly the same.
...
Answer: looking at the Q-Q plot it shows that several not few quantiles are away from

the Q-Q line and this indicates that the distribution is slightly right-skewed and

plotting the histogram of female knee diamater shows this.
