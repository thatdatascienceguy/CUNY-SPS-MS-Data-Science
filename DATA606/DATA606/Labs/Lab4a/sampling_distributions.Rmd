...
Answer: Looks slighty right skewed and also nearly normally distributed.
...
Answer:
```{r ames-sample-distribution, echo=TRUE}
hist(samp1)
```
The sample histogram looks mostly multimodial whereas the population distribution looks

more nearly normal than the sample.
...
Answer:
```{r samp2-mean, echo=TRUE}
samp2 <- sample(area, size = 50)
mean(samp2)
```

The sample is slightly different than the previous sample. If we were to take more samples

say 1000 instead of 100, we would get a more accurate and closer approximation to the actual population mean.
...
# number of elements in sample_means50
length(sample_means50)
```

There are 5000 elements in sample_means50. The sampling distribution is a nearly normal distribution.

The center is close to the true population mean which is around 1500.

If you instead do 50,000 iterations (50000 sample means), the distrbution will still be nearly normal if not
even closer and the sample mean/center wouldn't change much other than be very close to the true mean.
...
There are 100 elements in sample_means_small. Each element is the mean of a simple random sample of 50 elements
...
As the sample size get larger, the center becomes more approximate to the true population mean.

The spread (standard deviation) also gets more approximate to the true population standard deviation.
...
Answer:
```{r sample-price, echo=TRUE}
sample_means50_price <- sample(price, 50)
mean(sample_means50_price)
```
Using a point estimate, the guess would be the population mean of prices would be about 180000
...
Answer:
```{r sample-means-50-price, echo=TRUE}
sample_means50 <- rep(NA, 5000)
...
The shape of the sampling distribution is nearly normal.

Looking at the center, it would appear that the population mean of home prices

would be about $\mu$=180000
...
Answer:
```{r sample-means-150-price, echo=TRUE}
...
Like before, the distribution is nearly normal but a narrower spread than before.

I would expect the guess of the mean sale price to be 180000 same as before.
...
The 3rd distribution has a smaller spread (smaller standard deviation) than the 2nd one.

We would prefer to have a smaller spread as that slowly captures the true population standard deviation.
