...
```{r number-of-rows, echo=TRUE, eval=TRUE}
nrow(cdc)
```

```{r number-of-columns, echo=TRUE, eval=TRUE}
ncol(cdc)
```

```{r cdc-data-types, echo=TRUE, eval=TRUE}
str(cdc)
```

genhlth is a ordinal categorical variable
exerany is a categorical variable
hlthplan is a categorical variable
smoke100 is a categorical variable
height, weight and wtdesire are continous variables
age is a discrete variable
gender is a unordered categorical variable
...
```{r numerical-summary-height-age, eval=TRUE, echo=TRUE}
summary(cdc$height) # summary of height

summary(cdc$age) # summary of age
```

```{r relative-frequency-gender-exerany, eval=TRUE, echo=TRUE}
gender_relative <- table(cdc$gender)/nrow(cdc)
gender_relative

exerany_relative <- table(cdc$exerany)/nrow(cdc)
exerany_relative
```

How many males:
```{r num-males, eval=TRUE, echo=TRUE}
nrow(cdc[cdc$gender == 'm', ])
```

How many people in excellent health:
```{r num-excellent-health, eval=TRUE, echo=TRUE}
nrow(cdc[cdc$genhlth == 'excellent', ])
```
...
Answer: it shows that males are more likley to smoke more than females.
...
Answer:
```{r under23-and-smoke, eval=TRUE, echo=TRUE}
...
Answer: This box plot shows that the median bmi of the users in the cdc dataset
are about the same and contain many outliers. I also feel that regardless of health there is
not much difference in bmi.

The categorical variable I selected was 'exerany'. The reason for this is for one to change
their bmi, the weight can be changed, the height is fixed and someone who exercises is more likley to
lose weight and the median is smaller for people who exercise.
...
```{r weight-vs-height, eval=TRUE, echo=TRUE}
plot(cdc$weight, cdc$height)
```

The relationship is strongly linear and it shows that the weight and height have a 
linear relationship and that the taller someone is the more the weigh.
...
```{r var-wdiff, eval=TRUE, echo=TRUE}
...
```{r type-wdiff, eval=TRUE, echo=TRUE}
typeof(cdc$wdiff)
```
It is an integer value hence a discrete value.

If wdiff = 0, then the person has achieved the weight that they desired.
If wdiff < 0, then the person has weight greater than their desired weight and should
lose weight to get their desired weight.
If wdiff > 0 then the person has weight less than their desired weight and can eat more
workout etc.
...
```{r plot-wdiff, eval=TRUE, echo=TRUE}
hist(cdc$wdiff) # understand the frequency distribution
boxplot(cdc$wdiff) # understand the distribution, median and IQR
summary(cdc$wdiff)
```

looking at the box plot, we can see that is normally distributed (bell-shaped/Gaussian)
and shows many outliers and is narrowly spread and shows that more than half of people 
have the weight they desired by several pounds off.
...
```{r men-view-weight-than-woman, eval=TRUE, echo=TRUE}
boxplot(cdc$wdiff~ cdc$gender)
```
Using the boxplot shows that both men and women think simarliy about their weight
as the distributions are almost the same and usually have the weight they desire
...
Answer:
```{r get-mean-and-sd, eval=TRUE, echo=TRUE}
mu <- mean(cdc$weight)
sigma <- sd(cdc$weight)
one_sd <- c(mu-sigma, mu+sigma)
one_sd
```

The porportion of weights within one standard deviation (68% of the data) are between
129.6020 and 209.7639 lbs
